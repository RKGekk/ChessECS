...
public:
    ...
    Time() : startTime(std::chrono::high_resolution_clock::now()), milliseconds(startTime), millisecondDelta(0), seconds(0), secondDelta(0) {
    ...
    void update() {
        ...
        millisecondDelta = std::chrono::duration_cast<std::chrono::duration<float>>(millisecondsNew - milliseconds);
        milliseconds = millisecondsNew;
        seconds = std::chrono::duration_cast<std::chrono::duration<float>>(milliseconds - startTime).count() * 0.001f;
...
public:
    TimeScope(const char *message) : start(std::chrono::high_resolution_clock::now()), message(message), stopped(false) {}
