...
#define MOUSE_ACTION_TYPE_CASE(Action) case MouseAction::Action: \
    ...
    default:
        debug_error("unknown MouseAction %d", (int)action);
        break;
    }
    #undef MACRO
    ...
    switch (button) {
        BUTTON_SWITCH(_)
        default:
            debug_error("unknown mouse button  %d", button);
            break;
    }
}
...
float Input::get_wheel_impl() {
    ...
    if (dt < wheelHalfDecayTime) {
        ...
        wheel = glm::lerp(wheelData.lastValue, wheelData.targetValue, t);
    }
    else {
        float t = (dt - wheelHalfDecayTime) / wheelHalfDecayTime;
        wheel = glm::lerp(wheelData.targetValue, 0.f, t);
    }
    return wheel;
}
