...
namespace ecs {
	...
	struct EventDescription : QueryDescription {
		...
		EventDescription(const char* name, const std::vector<FunctionArgument>& args, EventHandler eventHandler, uint tags) : QueryDescription(name, args, false), eventHandler(eventHandler), tags(tags) {
	...
	struct SingleEventDescription : QueryDescription {
		...
		SingleEventDescription(const char* name, const std::vector<FunctionArgument>& args, EventHandler eventHandler, uint tags) : QueryDescription(name, args, false), eventHandler(eventHandler), tags(tags) {
	...
	std::pair<EntityId, Archetype&> add_entity(const vector<string_hash>& type_hashes);
	template<typename ...Args>
	EntityId create_entity(const ComponentInitializer<Args> &...args) {
		vector<string_hash> typeHashes = { get_type_description<EntityId>("eid").type_name_hash(), component_initializer_hash(args)... };
