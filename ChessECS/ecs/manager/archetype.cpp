...
namespace ecs {
	...
	Archetype::Archetype(int count, const string& synonim, int type_count) : components(), count(count), capacity(count), fullTypeDescriptions(type_count), synonim(synonim) {}

	Archetype::Archetype(const vector<string_hash>& type_hashes, int capacity, const string& synonim) : components(), count(0), capacity(capacity), fullTypeDescriptions(), synonim(synonim) {
	...
	void Archetype::destroy_entity(int index, bool with_swap) {
		...
		if (with_swap && index != count) {
			EntityId& eid = *components[get_type_description<EntityId>("eid").type_name_hash()].get_component<EntityId>(index);
