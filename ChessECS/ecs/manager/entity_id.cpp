...
namespace ecs {
	...
	EntityId::EntityId() : eid(nullptr) {}

	EntityId::EntityId(uint* id) : eid(id) {}

	EntityId::EntityId(uint* id, uint archetype, uint index) : eid(id) {
		migrate(archetype, index);
	}

	EntityId::EntityId(const EntityId& id) : eid(id.eid) {}

	EntityId::EntityId(EntityId&& id) : eid(id.eid) {
		id.eid = nullptr;
	}
