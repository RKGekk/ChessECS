...
namespace ecs
{
	...
	ComponentContainer::ComponentContainer() : data(), typeHash(-1), count(0) {}

	ComponentContainer::ComponentContainer(string_hash type_hash, string_hash type_name_hash, int capacity, int sizeOf) : data((capacity + binSize - 1) / binSize), typeHash(type_hash), typeNameHash(type_name_hash), count(0), capacity(data.size()* binSize), sizeOf(sizeOf)	{
