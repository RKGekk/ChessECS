...
namespace ecs {
	...
	RegisterTypeInfoRT register_type() {
		...
		if constexpr (is_singleton<T>::value) {
			SingletonTypeInfo::types().try_emplace(hash, SingletonTypeInfo{ {hash, string(nameOf<T>::value), sizeof(T), ecs::template_constructor<T>, ecs::template_copy_constructor<T>, ecs::template_destructor<T>}, template_singleton_instance<T> });
		}
		else {
			TypeInfo::types().try_emplace(hash, TypeInfo{ hash, string(nameOf<T>::value), sizeof(T), ecs::template_constructor<T>, ecs::template_copy_constructor<T>, ecs::template_destructor<T> });
		}
